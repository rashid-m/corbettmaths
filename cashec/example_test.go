...
func Example_signMessage() {
	...
	privKey, pubKey := cashec.PrivKeyFromBytes(cashec.S256(), pkBytes)
...
func Example_verifySignature() {
	...
	pubKey, err := cashec.ParsePubKey(pubKeyBytes, cashec.S256())
	if err != nil {
		fmt.Println(err)
		return
	}
	...
	signature, err := cashec.ParseSignature(sigBytes, cashec.S256())
	if err != nil {
		fmt.Println(err)
		return
	}
...
func Example_encryptMessage() {
	...
	message := "test message"
	ciphertext, err := cashec.Encrypt(pubKey, []byte(message))
	if err != nil {
		fmt.Println(err)
		return
	}
	...
	privKey, _ := cashec.PrivKeyFromBytes(cashec.S256(), pkBytes)

	// Try decrypting and verify if it's the same message.
	plaintext, err := cashec.Decrypt(privKey, ciphertext)
	if err != nil {
		fmt.Println(err)
		return
	}
