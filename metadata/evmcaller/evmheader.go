...
import (
	...
	"github.com/incognitochain/incognito-chain/dataaccessobject/rawdb_evmcaller"
...
func getEVMHeaderResultMultipleHosts(
	...
	hosts []string,
	minConfirmationBlocks int,
) (*rawdb_evmcaller.EVMHeaderResult, error) {
	evmHeaderResult := rawdb_evmcaller.NewEVMHeaderResult()
	// try with multiple hosts
	for _, host := range hosts {
...
func GetEVMHeaderResult(
	...
	isExisted, evmHeaderRes, err := rawdb_evmcaller.GetEVMHeaderResult(rawdb_evmcaller.GetEvmCallerDatabase(), networkPrefix, evmBlockHash)
	if err != nil {
		Logger.log.Errorf("An error occured during getting evm header result from db : %v", err)
	}
	...
	err = rawdb_evmcaller.StoreEVMHeaderResult(rawdb_evmcaller.GetEvmCallerDatabase(), networkPrefix, evmBlockHash, *evmHeaderRes)
	if err != nil {
		Logger.log.Errorf("An error occured during storing evm header result to db : %v", err)
	}
