...
func (addStakingMetadata AddStakingMetadata) ValidateTxWithBlockChain(tx Transaction, chainRetriever ChainRetriever, shardViewRetriever ShardViewRetriever, beaconViewRetriever BeaconViewRetriever, shardID byte, transactionStateDB *statedb.StateDB) (bool, error) {
	...
	// if ok, c, coinID, err := tx.GetTxBurnData(); (err == nil) && (ok) && (coinID.String() == common.PRVIDStr) {
	// 	if c.GetValue() != addStakingMetadata.AddStakingAmount {
	// 		return false, errors.New(fmt.Sprintf("Invalid staking amount, burned %v, amount in metadata %v", c.GetValue(), addStakingMetadata.AddStakingAmount))
	// 	}
	// } else {
	// 	return false, errors.New(fmt.Sprintf("Invalid tx burn data, error %v, ok %v, token ID %v", err, ok, coinID))
	// }
	return true, nil
}
