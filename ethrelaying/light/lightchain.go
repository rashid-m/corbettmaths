...
func (lc *LightChain) ValidateHeaderChain(chain []*types.Header, checkFreq int) (int, error) {
	return 0, nil
	i, err := lc.hc.ValidateHeaderChain(chain, checkFreq)
	if err != nil {
		fmt.Println("huhu 1 dm - ", err)
		return i, err
	}
	return i, nil
}
