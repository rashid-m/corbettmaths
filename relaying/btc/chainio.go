...
func (b *BlockChain) createChainStateV2(genBlk *wire.MsgBlock) error {
	// Create a new node from the genesis block and set it as the best node.
	genesisBlock := btcutil.NewBlock(genBlk)
	genesisBlock.SetHeight(0)
	header := &genesisBlock.MsgBlock().Header
	node := newBlockNode(header, nil)
	node.status = statusDataStored | statusValid
...
func (b *BlockChain) initChainStateV2(genBlk *wire.MsgBlock) error {
	// Determine the state of the chain database. We may need to initialize
	// everything from scratch or upgrade certain buckets.
	var initialized, hasBlockIndex bool
	err := b.db.View(func(dbTx database.Tx) error {
	...
	if !initialized {
		// At this point the database has not already been initialized, so
		// initialize both it and the chain state to the genesis block.
		return b.createChainStateV2(genBlk)
	}
	...
	err = b.db.View(func(dbTx database.Tx) error {
		...
		for ok := cursor.First(); ok; ok = cursor.Next() {
			...
			header.PrevBlock = genBlk.Header.PrevBlock
			...
			if lastNode == nil {
				...
				fmt.Println("genesis prev hash: ", genBlk.Header.PrevBlock.String())
				fmt.Println("genesis merkle root: ", genBlk.Header.MerkleRoot.String())
				fmt.Println("genesis timestamp: ", genBlk.Header.Timestamp.Unix())
				fmt.Println("genesis bits: ", genBlk.Header.Bits)
				fmt.Println("genesis nonce: ", genBlk.Header.Nonce)
				blockHash := header.BlockHash()
				genHash := genBlk.Header.BlockHash()
				fmt.Println("fuck: ", blockHash.String(), genHash.String())
				if !blockHash.IsEqual(&genHash) {
					return AssertError(fmt.Sprintf("initChainState: Expected "+
						"first entry in block index to be genesis block, "+
						"found %s", blockHash))
				}
