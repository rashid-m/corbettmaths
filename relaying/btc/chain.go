...
func (b *BlockChain) connectBlockV2(node *blockNode, block *btcutil.Block,
...
func (b *BlockChain) disconnectBlockV2(node *blockNode, block *btcutil.Block, view *UtxoViewpoint) error {
	// Make sure the node being disconnected is the end of the best chain.
	if !node.hash.IsEqual(&b.bestChain.Tip().hash) {
		return AssertError("disconnectBlock must be called with the " +
			"block at the end of the main chain")
...
func countSpentOutputsV2(block *btcutil.Block) int {
	// Exclude the coinbase transaction since it can't spend anything.
	var numSpent int
	for _, tx := range block.Transactions()[1:] {
		numSpent += len(tx.MsgTx().TxIn)
...
func (b *BlockChain) reorganizeChainV2(detachNodes, attachNodes *list.List) error {
	...
	for e := attachNodes.Front(); e != nil; e = e.Next() {
		...
		err = b.checkConnectBlockV2(n, block, view)
		if err != nil {
			...
			fmt.Println("Error with checkConnectBlockV2 inside reorganizeChainV2")
			return err
		}
	...
	for i, e := 0, detachNodes.Front(); e != nil; i, e = i+1, e.Next() {
		...
		err := b.disconnectBlockV2(n, block, view)
		if err != nil {
			return err
		}
	}
	...
	for i, e := 0, attachNodes.Front(); e != nil; i, e = i+1, e.Next() {
		...
		// Update the database and chain state.
		err := b.connectBlockV2(n, block, view)
		if err != nil {
			return err
		}
	}
...
func (b *BlockChain) connectBestChainV2(node *blockNode, block *btcutil.Block, flags BehaviorFlags) (bool, error) {
	...
	if parentHash.IsEqual(&b.bestChain.Tip().hash) {
		...
		err := b.checkConnectBlockV2(node, block, view)
		if err == nil {
		...
		if err != nil {
			fmt.Println("Error with checkConnectBlockV2 inside connectBestChainV2")
			return false, err
		}
		// Connect the block to the main chain.
		err = b.connectBlockV2(node, block, view)
		if err != nil {
	...
	err := b.reorganizeChainV2(detachNodes, attachNodes)
