...
if [ -z "$DBMODE" ]; then
  DBMODE="--syncmode archive";
else
  DBMODE="--syncmode ${DBMODE}";
fi

echo $FFSTORAGE
if [ -n "$FFSTORAGE" ] && [ "$FFSTORAGE" = "true" ]; then
  FFSTORAGE="--ffstorage"
else
  FFSTORAGE=""
fi
...
incp=$!
while ps -p $incp > /dev/null; do sleep 1s; done