...
func (bm *benchmarkCollector) Collect(chainID int, blkHeight uint64) {
	if blkHeight%bm.interval == 0 {
		if bm.colelctType == 0 {
			...
			memUsage := m.Sys >> 20
			str := fmt.Sprintf("%v,%v,%.0f,%v,%.6f,%v", chainID, blkHeight, bm.cpuUsage, memUsage, time.Since(bm.lastTime).Seconds(), time.Since(bm.lastTime).Milliseconds()/int64(bm.interval))
			bm.lastTime = time.Now()
			...
			str := fmt.Sprintf("%v,%v,%v", chainID, blkHeight, db)
			bm.fd.WriteString(str + "\n")
		}
