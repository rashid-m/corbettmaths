package connmanager

import "testing"

func TestConnManager_New(t *testing.T) {
	connManager := ConnManager{}.New()
}

func TestConnManager_Connect(t *testing.T) {

}

func TestConnManager_GetPeerConnOfPbk(t *testing.T) {

}

func TestConnManager_GetPeerConnOfShard(t *testing.T) {

}

func TestConnManager_GetPeerConnOfAll(t *testing.T) {

}

func TestConnManager_GetCurrentRoleShard(t *testing.T) {

}

func TestConnManager_GetPeerConnOfBeacon(t *testing.T) {

}

func TestConnManager_DiscoverPeers(t *testing.T) {

}

func TestConnManager_GetPeerId(t *testing.T) {

}

func TestConnManager_CheckForAcceptConn(t *testing.T) {

}

func TestConnManager_Start(t *testing.T) {

}

func TestConnManager_Stop(t *testing.T) {

}

func TestConnManager_UpdateConsensusState(t *testing.T) {

}

func TestConnManagerLogger_Init(t *testing.T) {

}
