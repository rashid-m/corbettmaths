...
var committerPool = sync.Pool{
	New: func() interface{} {
		return &committer{
			...
			sha: sha3.NewLegacyKeccak256().(common.KeccakState),
		}
	},
}
...
// Commit collapses a node down into a hash node and inserts it into the IntermediateWriter
func (c *committer) Commit(n node, db *IntermediateWriter) (hashNode, int, error) {
	if db == nil {
		return nil, 0, errors.New("no iw provided")
	}
...
// commit collapses a node down into a hash node and inserts it into the IntermediateWriter
func (c *committer) commit(n node, db *IntermediateWriter) (node, int, error) {
	...
	case *shortNode:
		...
		// The key needs to be copied, since we're delivering it to IntermediateWriter
...
func (c *committer) commitChildren(n *fullNode, db *IntermediateWriter) ([17]node, int, error) {
	var (
		committed int
		children  [17]node
	)
...
func (c *committer) store(n node, db *IntermediateWriter) node {
	// Larger nodes are replaced by their hash and stored in the IntermediateWriter.
	var (
		hash, _ = n.cache()
		size    int
	)
	...
	if c.leafCh != nil {
		...
		// No leaf-callback used, but there's still a IntermediateWriter. Do serial
		// insertion
		db.lock.Lock()
...
func (c *committer) commitLoop(db *IntermediateWriter) {
