...
type Sync struct {
	database incdb.KeyValueReader     // Persistent database to check for existing entries
...
func NewSync(root common.Hash, database incdb.KeyValueReader, callback LeafCallback) *Sync {
	ts := &Sync{
		database: database,
...
func (s *Sync) AddSubTrie(root common.Hash, path []byte, parent common.Hash, callback LeafCallback) {
	...
	blob := rawdbv2.ReadTrieNode(s.database, root)
	if len(blob) > 0 {
		return
	}
...
func (s *Sync) Commit(dbw incdb.Batch) error {
	...
	for key, value := range s.membatch.nodes {
		rawdbv2.WriteTrieNode(dbw, key, value)
	}
	...
	for _, child := range children {
		...
		if req.callback != nil {
			if node, ok := (child.node).(valueNode); ok {
				var paths [][]byte
				if len(child.path) == 2*common.HashSize {
					paths = append(paths, hexToKeybytes(child.path))
				} else if len(child.path) == 4*common.HashSize {
					paths = append(paths, hexToKeybytes(child.path[:2*common.HashSize]))
					paths = append(paths, hexToKeybytes(child.path[2*common.HashSize:]))
				}
		...
		if node, ok := (child.node).(hashNode); ok {
			...
			if blob := rawdbv2.ReadTrieNode(s.database, hash); len(blob) > 0 {
				continue
			}
