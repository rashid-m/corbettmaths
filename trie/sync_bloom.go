...
func NewSyncBloom(memory uint64, database incdb.Iteratee) *SyncBloom {
	...
	Logger.log.Info("Allocated fast sync bloom", "size", common.StorageSize(memory*1024*1024))
...
func (b *SyncBloom) init(database incdb.Iteratee) {
	...
	for it.Next() && atomic.LoadUint32(&b.closed) == 0 {
		...
		if key := it.Key(); len(key) == common.HashSize {
