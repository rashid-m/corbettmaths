...
func (walletService WalletService) SubmitKey(keyStr string) (jsonresult.Temp, *RPCError) {
	...
	if err==nil{
		...
		if err!=nil || len(keySlice)!=64{
			return jsonresult.Temp{}, NewRPCError(InvalidSenderViewingKeyError, errors.New("OTA key must be hex-encoded 64 bytes"))
		}
		var b [64]byte
		copy(b[:], keySlice)
	...
	result := jsonresult.Temp{}
	
	err = walletService.BlockChain.SubmitOTAKey(otaKey, shardIDSender)
	if err != nil {
		return jsonresult.Temp{}, NewRPCError(UnexpectedError, err)
	}
