	...
	case transaction.CustomTokenTransfer:
		{
			...
			if !existed {
				return nil, nil, nil, NewRPCError(BuildPrivacyTokenParamError, fmt.Errorf("Token %+v not existed", tokenID))
			}
...
func (txService TxService) GetTransactionByHash(txHashStr string) (*jsonresult.TransactionDetail, *RPCError) {
	...
	shardID, blockHash, index, tx, err := txService.BlockChain.GetTransactionByHashV2(*txHash)
	if err != nil {
	...
	for shardID, txHashs := range listTxsHash {
		for _, txHash := range txHashs {
			...
			if len(keySet.ReadonlyKey.Rk) != 0 {
				_, blockHash, _, txDetail, _ := txService.BlockChain.GetTransactionByHashV2(txHash)
