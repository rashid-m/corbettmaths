...
func (txService TxService) CreateRawStakingTransaction(params interface{}, shardAmountStaking uint64) (string, []byte, byte, *RPCError){
	...
	if errNewParam != nil{
		return "", nil, byte(0), NewRPCError(RPCInvalidParamsError, errNewParam)
	}
	...
	if !ok {
		return "", nil, byte(0), NewRPCError(RPCInvalidParamsError, fmt.Errorf("Invalid Data For Staking Transaction %+v", paramsArray[4]))
	}

	stakingType, ok := data["StakingType"].(float64)
	if !ok {
		return "", nil, byte(0), NewRPCError(RPCInvalidParamsError, fmt.Errorf("Invalid Staking Type For Staking Transaction %+v",  data["StakingType"]))
	}

	candidatePaymentAddress, ok := data["CandidatePaymentAddress"].(string)
	if !ok {
		return "", nil, byte(0), NewRPCError(RPCInvalidParamsError,  fmt.Errorf("Invalid Producer Payment Address for Staking Transaction %+v", data["CandidatePaymentAddress"]))
	}

	// Get private seed, a.k.a mining key
	privateSeed, ok := data["PrivateSeed"].(string)
	if !ok {
		return "", nil, byte(0), NewRPCError(RPCInvalidParamsError, fmt.Errorf("Invalid Private Seed For Staking Transaction %+v", data["PrivateSeed"]))
	}
	...
	if !ok {
		return "", nil, byte(0), NewRPCError(RPCInvalidParamsError, fmt.Errorf("Invalid Reward Receiver Payment Address For Staking Transaction %+v", data["RewardReceiverPaymentAddress"]))
	}

	//Get auto staking flag
	autoReStaking, ok := data["AutoReStaking"].(bool)
	if !ok {
		return "", nil, byte(0), NewRPCError(RPCInvalidParamsError, fmt.Errorf("Invalid auto restaking flag %+v", data["AutoReStaking"]))
	}
	...
	tx, err := txService.BuildRawTransaction(createRawTxParam, stakingMetadata)
	if err.(*RPCError) != nil {
		Logger.log.Critical(err)
		Logger.log.Debugf("handleCreateRawStakingTransaction result: %+v, err: %+v", nil, err)
...
func (txService TxService) CreateRawStopAutoStakingTransaction(params interface{}) (string, []byte, byte, *RPCError){
	...
	if !ok {
		return "", nil, byte(0), NewRPCError(RPCInvalidParamsError, fmt.Errorf("Invalid Staking Type For Staking Transaction %+v", data["StopAutoStakingType"]))
	}
