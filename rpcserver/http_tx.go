...
func (httpServer *HttpServer) handleCreateRawStakingTransaction(params interface{}, closeChan <-chan struct{}) (interface{}, *RPCError) {
	...
	if !ok {
		return nil, NewRPCError(RPCInvalidParamsError, fmt.Errorf("Invalid Reward Receiver Payment Address for Staking Transaction %+v", paramsArray[7]))
	}

	//Get auto staking flag
	autoReStaking, ok := paramsArray[8].(bool)
	if !ok {
		return nil, NewRPCError(RPCInvalidParamsError, fmt.Errorf("Invalid auto restaking flag %+v", paramsArray[8]))
	}
