...
func (httpServer *HttpServer) handleCheckSolTxSigIssued(params interface{}, closeChan <-chan struct{}) (interface{}, *rpcservice.RPCError) {
	arrayParams := common.InterfaceSlice(params)
	if arrayParams == nil || len(arrayParams) < 1 {
		return nil, rpcservice.NewRPCError(rpcservice.RPCInvalidParamsError, errors.New("param must be an array at least 1 element"))
	}
	...
	issued, err := httpServer.blockService.CheckSolTxSigIssued(data)
	if err != nil {
		return false, rpcservice.NewRPCError(rpcservice.UnexpectedError, err)
	}
	return issued, nil
...
func (httpServer *HttpServer) handleCreateRawTxWithIssuingSOLReq(params interface{}, closeChan <-chan struct{}) (interface{}, *rpcservice.RPCError) {
	// parse and validate sanity params
	arrayParams := common.InterfaceSlice(params)
	if arrayParams == nil || len(arrayParams) < 5 {
		return nil, rpcservice.NewRPCError(rpcservice.RPCInvalidParamsError, errors.New("param must be an array at least 5 elements"))
	}
	...
	data, ok := arrayParams[4].(map[string]interface{})
	if !ok {
		return nil, rpcservice.NewRPCError(rpcservice.RPCInvalidParamsError, errors.New("params metadata is invalid"))
	}

	txSig, ok := data["TxSig"].(string)
	if !ok {
		return nil, rpcservice.NewRPCError(rpcservice.RPCInvalidParamsError, errors.New("TxSig is invalid"))
	}

	incTokenID, err := common.Hash{}.NewHashFromStr(data["IncTokenID"].(string))
	if err != nil {
		return nil, rpcservice.NewRPCError(rpcservice.RPCInvalidParamsError, errors.New("IncTokenID is invalid"))
	}

	// create metadata
	meta, err := metadata.NewIssuingSOLRequest(txSig, *incTokenID, metadata.IssuingSOLRequestMeta)
	if err != nil {
		rpcErr := rpcservice.NewRPCError(rpcservice.UnexpectedError, err)
		Logger.log.Error(rpcErr)
		return nil, rpcErr
...
func (httpServer *HttpServer) handleCreateAndSendTxWithIssuingSOLReq(params interface{},
	closeChan <-chan struct{}) (interface{}, *rpcservice.RPCError) {
	data, err := httpServer.handleCreateRawTxWithIssuingSOLReq(params, closeChan)
	if err != nil {
		return nil, rpcservice.NewRPCError(rpcservice.UnexpectedError, err)
	}
	tx := data.(jsonresult.CreateTransactionResult)
...
func processBurningSOLReq(
	burningMetaType int,
	params interface{},
	closeChan <-chan struct{},
	httpServer *HttpServer,
) (interface{}, *rpcservice.RPCError) {
	// parse and verify sanity params
	arrayParams := common.InterfaceSlice(params)
	if arrayParams == nil || len(arrayParams) < 5 {
		return nil, rpcservice.NewRPCError(rpcservice.RPCInvalidParamsError, errors.New("param must be an array at least 5 elements"))
	}
	...
	keyWallet, err := wallet.Base58CheckDeserialize(senderPrivateKeyParam)
	if err != nil || len(keyWallet.KeySet.PrivateKey) == 0 {
		return nil, rpcservice.NewRPCError(rpcservice.RPCInvalidParamsError, errors.New("private key is invalid"))
	}
	...
	// create metadata
	meta, err2 := rpcservice.NewBurningSOLRequestMetadata(
		tokenReceivers,
		tokenID,
		remoteAddress,
		burningMetaType,
...
func (httpServer *HttpServer) handleCreateRawTxWithBurningSOLReq(params interface{}, closeChan <-chan struct{}) (interface{}, *rpcservice.RPCError) {
	return processBurningSOLReq(metadata.BurningSOLRequestMeta, params, closeChan, httpServer)
}

func (httpServer *HttpServer) handleCreateAndSendBurningSOLRequest(params interface{}, closeChan <-chan struct{}) (interface{}, *rpcservice.RPCError) {
	data, err := httpServer.handleCreateRawTxWithBurningSOLReq(params, closeChan)
	if err != nil {
		return nil, rpcservice.NewRPCError(rpcservice.UnexpectedError, err)
	}
...
func (httpServer *HttpServer) handleCreateRawTxWithBurningSOLForDepositToSCReq(params interface{}, closeChan <-chan struct{}) (interface{}, *rpcservice.RPCError) {
	return processBurningSOLReq(metadata.BurningSOLForDepositToSCRequestMeta, params, closeChan, httpServer)
}

func (httpServer *HttpServer) handleCreateAndSendBurningSOLForDepositToSCRequest(params interface{}, closeChan <-chan struct{}) (interface{}, *rpcservice.RPCError) {
	data, err := httpServer.handleCreateRawTxWithBurningSOLForDepositToSCReq(params, closeChan)
	if err != nil {
		return nil, rpcservice.NewRPCError(rpcservice.UnexpectedError, err)
	}
	tx := data.(jsonresult.CreateTransactionResult)
