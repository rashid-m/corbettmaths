...
func (rpcServer RpcServer) handleCreateAndSendTxWithETHHeadersRelaying(params interface{}, closeChan <-chan struct{}) (interface{}, *RPCError) {
	data, err := rpcServer.handleCreateRawTxWithETHHeadersRelaying(params, closeChan)
	if err != nil {
		return nil, err
	}
	tx := data.(jsonresult.CreateTransactionResult)
...
func (rpcServer RpcServer) handleCreateRawTxWithIssuingETHReq(params interface{}, closeChan <-chan struct{}) (interface{}, *RPCError) {
	arrayParams := common.InterfaceSlice(params)
...
func (rpcServer RpcServer) handleCreateAndSendTxWithIssuingETHReq(params interface{}, closeChan <-chan struct{}) (interface{}, *RPCError) {
	data, err := rpcServer.handleCreateRawTxWithIssuingETHReq(params, closeChan)
	if err != nil {
		return nil, err
	}
	tx := data.(jsonresult.CreateTransactionResult)
...
func (rpcServer RpcServer) handleGetRelayedETHHeader(params interface{}, closeChan <-chan struct{}) (interface{}, *RPCError) {
	arrayParams := common.InterfaceSlice(params)
	// blockNum := uint64(arrayParams[0].(float64))
	// header := rpcServer.config.BlockChain.GetLightEthereum().GetLightChain().GetHeaderByNumber(blockNum)
	headerHash := rCommon.HexToHash(arrayParams[0].(string))
	header := rpcServer.config.BlockChain.GetLightEthereum().GetLightChain().GetHeaderByHash(headerHash)
	return header, nil
}
