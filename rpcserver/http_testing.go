...
func (httpServer *HttpServer) handleGetProposerIndex(params interface{}, closeChan <-chan struct{}) (interface{}, *rpcservice.RPCError) {
	...
	tempCommittee, committeIndex := blsbft.GetProposerByTimeSlotFromCommitteeList(int(tempShardID), common.CalculateTimeSlot(time.Now().Unix()), shardBestState.GetShardCommittee())
