...
func (httpServer *HttpServer) handleCreateRawTxWithPushBNBHeaderChain(params interface{}, closeChan <-chan struct{}) (interface{}, *rpcservice.RPCError) {
	arrayParams := common.InterfaceSlice(params)
	if len(arrayParams) < 5 {
		return nil, rpcservice.NewRPCError(rpcservice.RPCInvalidParamsError, errors.New("Param array must be at least 5"))
	}
	...
	meta, _ := metadata.NewPortalPushBNBHeaderRelaying(
		metadata.PortalPushHeaderRelayingMeta,
		senderAddress,
		header,
	)
...
func (httpServer *HttpServer) handleCreateAndSendTxWithPushBNBHeaderChain(params interface{}, closeChan <-chan struct{}) (interface{}, *rpcservice.RPCError) {
	data, err := httpServer.handleCreateRawTxWithPushBNBHeaderChain(params, closeChan)
	if err != nil {
		return nil, rpcservice.NewRPCError(rpcservice.UnexpectedError, err)
	}
	tx := data.(jsonresult.CreateTransactionResult)
