...
func (portalUserRegister PortalUserRegister) ValidateSanityData(chainRetriever basemeta.ChainRetriever, shardViewRetriever basemeta.ShardViewRetriever, beaconViewRetriever basemeta.BeaconViewRetriever, beaconHeight uint64, txr basemeta.Transaction) (bool, bool, error) {
	...
	if err != nil {
		return false, false, basemeta.NewMetadataTxError(basemeta.IssuingRequestNewIssuingRequestFromMapEror, errors.New("ContributorAddressStr incorrect"))
	}
...
func (portalUserRegister PortalUserRegister) ValidateMetadataByItself() bool {
	return portalUserRegister.Type == basemeta.PortalRequestPortingMeta || portalUserRegister.Type == basemeta.PortalRequestPortingMetaV3
}
...
func (portalUserRegister *PortalUserRegister) BuildReqActions(tx basemeta.Transaction, chainRetriever basemeta.ChainRetriever, shardViewRetriever basemeta.ShardViewRetriever, beaconViewRetriever basemeta.BeaconViewRetriever, shardID byte, shardHeight uint64) ([][]string, error) {
	if portalUserRegister.Type == basemeta.PortalRequestPortingMeta {
		...
		action := []string{strconv.Itoa(basemeta.PortalRequestPortingMeta), actionContentBase64Str}
		return [][]string{action}, nil
	} else if portalUserRegister.Type == basemeta.PortalRequestPortingMetaV3 {
		...
		actionContentBase64Str := base64.StdEncoding.EncodeToString(actionContentBytes)
		action := []string{strconv.Itoa(basemeta.PortalRequestPortingMetaV3), actionContentBase64Str}
		return [][]string{action}, nil
	}
	return nil, nil
}
...
func (portalUserRegister *PortalUserRegister) CalculateSize() uint64 {
	return basemeta.CalculateSize(portalUserRegister)
}
