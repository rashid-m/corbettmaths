...
func (blockchain *BlockChain) processPortalRedeemRequest(
	...
	var actionData metadata2.PortalRedeemRequestContent
	...
	if reqStatus == common.PortalRedeemRequestAcceptedChainStatus {
		...
		redeemRequestStatus := metadata2.PortalRedeemRequestStatus{
		...
		redeemRequestByTxIDStatus := metadata2.PortalRedeemRequestStatus{
...
func (blockchain *BlockChain) processPortalReqMatchingRedeem(
	...
	// unmarshal instructions content
	var actionData metadata2.PortalReqMatchingRedeemContent
	err := json.Unmarshal([]byte(instructions[3]), &actionData)
	if err != nil {
		Logger.log.Errorf("Can not unmarshal instruction content %v - Error %v\n", instructions[3], err)
		return nil
	...
	if reqStatus == common.PortalReqMatchingRedeemAcceptedChainStatus {
		...
		redeemRequest := metadata2.PortalRedeemRequestStatus{
		...
		redeemRequestByTxIDStatus := metadata2.PortalReqMatchingRedeemStatus{
...
func (blockchain *BlockChain) processPortalUnlockCollateral(
	...
	// unmarshal instructions content
	var actionData metadata2.PortalRequestUnlockCollateralContent
	var err error
	err = json.Unmarshal([]byte(instructions[3]), &actionData)
	if err != nil {
		Logger.log.Errorf("Can not unmarshal instruction content %v - Error %v\n", instructions[3], err)
	...
	if reqStatus == common.PortalReqUnlockCollateralAcceptedChainStatus {
		...
		reqUnlockCollateralTrackData := metadata2.PortalRequestUnlockCollateralStatus{
