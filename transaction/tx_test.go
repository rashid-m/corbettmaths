...
func TestUnmarshalJSON(t *testing.T) {
	...
	tx, err := BuildCoinBaseTxByCoinID(
		NewBuildCoinBaseTxByCoinIDParams(
			&paymentAddress, 10, &key.KeySet.PrivateKey,
			nil, responseMeta, common.Hash{}, NormalCoinType, "PRV", 0, nil,
		),
	)
	assert.Equal(t, nil, err)
	assert.NotEqual(t, nil, tx)
	for _, outCoin := range tx.GetProof().GetOutputCoins() {
		assert.Equal(t, false, outCoin.IsEncrypted())
	}
	assert.Equal(t, uint64(10), tx.GetProof().GetOutputCoins()[0].GetValue())
	...
	assert.Equal(t, uint64(10), tx1.Proof.GetOutputCoins()[0].GetValue())
