...
func updateRole(shardValidators, beaconValidators map[string]*Validator, cs *jsonresult.CommiteeState) error {
			...
			for _, c := range v {
				...
				if beaconValidators[k].Role != BeaconCommitteeRole {
					if beaconValidators[k].Role != BeaconPendingRole {
						return fmt.Errorf("key %s is not BeaconPendingRole when switch to BeaconCommitteeRole", shortKey(beaconValidators[k].MiningPublicKey))
					}
				...
				if k, found := svs[c]; found {
					if shardValidators[k].Role != ShardCommitteeRole {
						if shardValidators[k].Role != ShardPendingRole {
							return fmt.Errorf("key %s is not ShardPendingRole when switch to ShardCommitteeRole", shortKey(shardValidators[k].MiningPublicKey))
						}
			...
			for _, c := range v {
				...
				if beaconValidators[k].Role != BeaconPendingRole {
					if beaconValidators[k].Role != BeaconWaitingRole {
						return fmt.Errorf("key %s is not BeaconWaitingRole when switch to BeaconPendingRole", shortKey(beaconValidators[k].MiningPublicKey))
					}
				...
				if k, found := bvs[c]; found {
					if beaconValidators[k].Role != ShardPendingRole {
						if beaconValidators[k].Role != ShardSyncingRole {
							return fmt.Errorf("key %s is not ShardSyncingRole when switch to ShardPendingRole", shortKey(beaconValidators[k].MiningPublicKey))
						}
				...
				if k, found := svs[c]; found {
					if shardValidators[k].Role != ShardPendingRole {
						if shardValidators[k].Role != ShardSyncingRole {
							return fmt.Errorf("key %s is not ShardSyncingRole when switch to ShardPendingRole", shortKey(shardValidators[k].MiningPublicKey))
						}
			...
			if k, found := bvs[c]; found {
				if beaconValidators[k].Role != ShardSyncingRole {
					if beaconValidators[k].Role != ShardCandidateRole {
						return fmt.Errorf("key %s is not ShardCandidateRole when switch to ShardSyncingRole", shortKey(beaconValidators[k].MiningPublicKey))
					}
			...
			if k, found := svs[c]; found {
				if shardValidators[k].Role != ShardSyncingRole {
					if shardValidators[k].Role != ShardCandidateRole {
						return fmt.Errorf("key %s is not ShardCandidateRole when switch to ShardSyncingRole", shortKey(shardValidators[k].MiningPublicKey))
					}
		...
		if k, found := bvs[v]; found {
			if beaconValidators[k].Role != ShardCandidateRole {
				if beaconValidators[k].Role != NormalRole {
					return fmt.Errorf("key %s is not NormalRole when switch to ShardCandidateRole", shortKey(beaconValidators[k].MiningPublicKey))
				}
		...
		if k, found := svs[v]; found {
			if shardValidators[k].Role != ShardCandidateRole {
				if shardValidators[k].Role != NormalRole {
					return fmt.Errorf("key %s is not NormalRole when switch to ShardCandidateRole", shortKey(shardValidators[k].MiningPublicKey))
				}
	...
	for _, v := range cs.BeaconWaiting {
		if k, found := bvs[v]; found {
			if beaconValidators[k].Role != BeaconWaitingRole {
				if beaconValidators[k].Role != ShardCommitteeRole {
					return fmt.Errorf("key %s is not ShardCommitteeRole when switch to BeaconWaitingRole", shortKey(beaconValidators[k].MiningPublicKey))
				}
