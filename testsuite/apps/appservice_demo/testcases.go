...
func updateRole(shardValidators, beaconValidators map[string]*Validator, cs *jsonresult.CommiteeState, isInit bool) error {
	...
	for i, v := range cs.Substitute {
		if i == -1 {
			for _, c := range v {
				k := bvs[c]
				role := beaconValidators[k].Role
				if role != BeaconPendingRole && role != BeaconCommitteeRole && role != BeaconWaitingRole && !isInit {
	...
	for _, v := range cs.BeaconLocking {
		if k, found := bvs[v]; found {
			role := beaconValidators[k].Role
			if role != BeaconLockingRole && role != ShardCommitteeRole && role != BeaconWaitingRole && role != BeaconPendingRole && role != BeaconCommitteeRole && !isInit {
