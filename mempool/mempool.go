...
func (tp *TxPool) checkFees(
	...
	if txType == common.TxCustomTokenPrivacyType {
		...
		if meta != nil {
			ok := meta.CheckTransactionFee(tx, limitFee, beaconHeight, tp.config.BlockChain.BestState.Shard[shardID].GetCopiedTransactionStateDB())
			if !ok {
		...
		if feePToken > 0 {
			feePTokenToNativeTokenTmp, err := metadata.ConvertPrivacyTokenToNativeToken(feePToken, tokenID, beaconHeight, tp.config.BlockChain.BestState.Shard[shardID].GetCopiedTransactionStateDB())
			if err != nil {
				Logger.log.Errorf("ERROR: %+v", NewMempoolTxError(RejectInvalidFee,
		...
		if limitFee > 0 {
			meta := tx.GetMetadata()
			if meta != nil {
				ok := tx.GetMetadata().CheckTransactionFee(tx, limitFee, beaconHeight, tp.config.BlockChain.BestState.Shard[shardID].GetCopiedTransactionStateDB())
				if !ok {
					Logger.log.Errorf("ERROR: %+v", NewMempoolTxError(RejectInvalidFee,
...
func (tp *TxPool) validateTransaction(tx metadata.Transaction, beaconHeight int64) error {
	...
	shardID = common.GetShardIDFromLastByte(tx.GetSenderAddrLastByte())
	now = time.Now()
	validated, errValidateTxByItself := tx.ValidateTxByItself(tx.IsPrivacy(), tp.config.BlockChain.BestState.Shard[shardID].GetCopiedTransactionStateDB(), tp.config.BlockChain, shardID)
	go metrics.AnalyzeTimeSeriesMetricData(map[string]interface{}{
		metrics.Measurement:      metrics.TxPoolValidationDetails,
		metrics.MeasurementValue: float64(time.Since(now).Seconds()),
