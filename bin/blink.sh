...
CONFIGS[NUM_INDEXER_WORKERS]="101"
...
HINTS[RPC_PORT]="! RPC port, should be left as default (8334),
   The first node uses this port, the next one uses port+1 and so on: "
HINTS[NODE_PORT]="! Node port, should be left as default (9334),
   The first node uses this port, the next one uses port+1 and so on: "
HINTS[NUM_INDEXER_WORKERS]="! Number of coin indexer worker, default = 100. To disable this, set it to 0: "
...
cat << EOF
   $(basename $0) [-u|h] [-y 1|2]
      ...
      -y: Run setup with default settings, non-interactive mode. [-y 1] to install new server, [-y 2] to update
...
EOF
  ...
  while cat << EOF
! 1: Install on a new server
! 2: Update current Updater service
EOF
  do
    read interactive_mode
    if [[ $interactive_mode == 1 || $interactive_mode == 2 ]]; then break; fi
  done
else
cat << EOF
=======================================================
