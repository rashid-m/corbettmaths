...
run()
{
  ...
  docker run --restart=always --net inc_net -p 9334:9334 -p 9433:9433 -e GETH_NAME=inc_kovan -e PRIVATEKEY=${privatekey} -v $PWD/${dataDir}:/data -d --name inc_miner incognitochain/incognito:${latest_tag}
}
