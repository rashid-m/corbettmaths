...
run()
{
  ...
  docker run --restart=always --net inc_net -p 9334:9334 -p 9433:9433 -e BOOTNODE_IP=$bootnode -e GETH_NAME=inc_kovan -e MININGKEY=${validator_key} -e TESTNET=true -v $PWD/${data_dir}:/data -d --name inc_miner incognitochain/incognito:${latest_tag}
