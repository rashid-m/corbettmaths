...
func TestFinishSyncManager_AddFinishedSyncValidators(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			manager.AddFinishedSyncValidators(tt.args.newFinishedSyncValidators, tt.args.syncPool, tt.args.shardID, 0)
...
func TestFinishSyncManager_Instructions(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			manager := &FinishSyncMsgPool{
				FinishedSyncValidators: tt.fields.finishedSyncValidators,
				ReceiveTime:            tt.fields.receiveTime,
				mu:                     tt.fields.mu,
			}
			got := manager.Instructions(tt.args.allSyncPool, 10000)
