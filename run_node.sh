...
# Shard 0
if [ "$1" == "shard0-0" ]; then
go run *.go --discoverpeersaddress "127.0.0.1:9330" --privatekey "112t8rqJHgJp2TPpNpLNx34aWHB5VH5Pys3hVjjhhf9tctVeCNmX2zQLBqzHau6LpUbSV52kXtG2hRZsuYWkXWF5kw2v24RJq791fWmQxVqy" --nodemode "auto" --datadir "data/shard0-0" --listen "127.0.0.1:9433" --externaladdress "127.0.0.1:9433" --norpcauth --rpclisten "127.0.0.1:9334" --enablewallet --wallet "wallet1" --walletpassphrase "12345678" --walletautoinit --relayshards "0" --txpoolmaxtx 100000 --metricurl="http://128.199.96.206:8086/write?db=mydb"
fi
if [ "$1" == "shard0-1" ]; then
...
if [ "$1" == "shard0-2" ]; then
go run *.go --discoverpeersaddress "127.0.0.1:9330" --privatekey "112t8rnYtBaL6m37KBSPkyFp2A2V3Fbxg8ty2yuoRGscgj1PUZ82M3592x1CosxD23d9Pp4FvvTCBPYArrdp6Gqyfvynn3USmkgRGMVjKcbx" --nodemode "auto" --datadir "data/shard0-2" --listen "127.0.0.1:9435" --externaladdress "127.0.0.1:9435" --norpcauth --rpclisten "127.0.0.1:9336" --relayshards "0"
fi
# Shard 1
if [ "$1" == "shard1-0" ]; then
go run *.go --discoverpeersaddress "127.0.0.1:9330" --privatekey "112t8rnXVJFp1p3VQocZ21pcwvraAU9UHohp8TRt9CrN6e26VKxuFwr3CCDrbhQdmoPjakWgU4oMRHiihnsqN4tGxckmxVzoHGnAYrskyXW3" --nodemode "auto" --datadir "data/shard1-0" --listen "127.0.0.1:9443" --externaladdress "127.0.0.1:9443" --norpcauth --rpclisten "127.0.0.1:9337" --enablewallet --wallet "wallet3" --walletpassphrase "12345678" --walletautoinit
fi
if [ "$1" == "shard1-1" ]; then
