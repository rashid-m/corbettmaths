...
func makeBasicHost(listenPort int, secio bool, randseed int64) (host.Host, error) {
	...
	log.Printf("I am %s\n", fullAddr)
	if secio {
		log.Printf("Now run \"go run example.go -l %d -d %s -secio\" on a different terminal\n", listenPort+1, fullAddr)
	} else {
		log.Printf("Now run \"go run example.go -l %d -d %s\" on a different terminal\n", listenPort+1, fullAddr)
	}
...
func main1() {
	...
	if *target == "" {
		...
		// Set a stream handler on host A. /peer/1.0.0 is
		// a user-defined protocol name.
		ha.SetStreamHandler("/peer/1.0.0", handleStream)
