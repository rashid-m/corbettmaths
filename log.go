...
var (
	...
	rpcLogger              = backendLog.Logger("RPC log", true)
	rpcServiceLogger       = backendLog.Logger("RPC service log", true)
	rpcServiceBridgeLogger = backendLog.Logger("RPC service DeBridge log", true)
	netsyncLogger          = backendLog.Logger("Netsync log", true)
	peerLogger             = backendLog.Logger("Peer log", true)
	dbLogger               = backendLog.Logger("Database log", true)
	dbmpLogger             = backendLog.Logger("Mempool Persistence DB log", true)
	walletLogger           = backendLog.Logger("Wallet log", true)
	blockchainLogger       = backendLog.Logger("BlockChain log", true)
	consensusLogger        = backendLog.Logger("Consensus log", true)
	mempoolLogger          = backendLog.Logger("Mempool log", true)
	appServiceLogger       = backendLog.Logger("AppService log", true)
	storageLogger          = backendLog.Logger("Storage log", true)
	dataLogger             = backendLog.Logger("Data log", true)
	...
	bridgeLogger           = backendLog.Logger("DeBridge log", true)
	metadataLogger         = backendLog.Logger("Metadata log", true)
	trieLogger             = backendLog.Logger("Trie log", true)
	peerv2Logger           = backendLog.Logger("Peerv2 log", true)
	relayingLogger         = backendLog.Logger("Relaying log", true)
	wrapperLogger          = backendLog.Logger("Wrapper log", true)
	daov2Logger            = backendLog.Logger("DAO log", true)
	btcRelayingLogger      = backendLog.Logger("BTC relaying log", true)
	synckerLogger          = backendLog.Logger("Syncker log ", true)
)
