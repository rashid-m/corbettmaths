...
type ConstitutionHelper interface {
	GetStartedNormalVote(generator *BlkTmplGenerator, shardID byte) int32
...
// 			addable := blockgen.checkIssuingReqTx(shardID, tx)
// 			if !addable {
// 				txToRemove = append(txToRemove, tx)
// 				continue
// 			}
...
// func (blockgen *BlkTmplGenerator) neededNewDCBConstitution(shardID byte) bool {
...
// 	return false
// }
// func (blockgen *BlkTmplGenerator) neededNewGOVConstitution(shardID byte) bool {
// 	BestBlock := blockgen.chain.BestState[shardID].BestBlock
...
// 	for blockHeight := ConstitutionHelper.GetStartedNormalVote(blockgen, shardID); blockHeight < BestBlock.Header.Height; blockHeight += 1 {
...
// 		txResponse, err = buildResponseForToken(tx, valuesInToken, sellingAsset[:], rt, shardID, unspentTokenMap, saleData.SaleID, mint)
// 	}
// 	return txResponse, err
// }

// func (blockgen *BlkTmplGenerator) processCrowdsale(sourceTxns []*metadata.TxDesc, rt []byte, shardID byte, producerPrivateKey *privacy.SpendingKey) ([]*transaction.TxCustomToken, []metadata.Transaction, error) {
...
// 		}
// 		txResponse, err := blockgen.buildResponseForCrowdsale(tx, saleDataMap, unspentTokenMap, rt, shardID, metaRequest.SaleID, producerPrivateKey)
// 		if err != nil {
// 			txsToRemove = append(txsToRemove, tx)
