...
func (blockchain *BlockChain) buildBridgeInstructionsV2(stateDB *statedb.StateDB, shardID byte, shardBlockInstructions [][]string, beaconHeight uint64) ([][]string, error) {
	instructions := [][]string{}
	for _, inst := range shardBlockInstructions {
		...
		case metadata.BurningRequestMeta:
			...
			burningConfirm, err = buildBurningConfirmInstV2(stateDB, inst, beaconHeight)
...
func buildBurningConfirmInstV2(stateDB *statedb.StateDB, inst []string, height uint64) ([]string, error) {
	...
	tokenID, err := findExternalTokenIDV2(stateDB, &md.TokenID)
	if err != nil {
		return nil, err
	}
...
func findExternalTokenIDV2(stateDB *statedb.StateDB, tokenID *common.Hash) ([]byte, error) {
