...
func (blockchain *BlockChain) getValidatorsFromCacheByEpoch(
	...
	for _, key := range blockchain.committeeByEpochCache.Keys() {
		Logger.log.Infof("key in committee cache %v", key.(string))
	}
...
func (blockchain *BlockChain) getBeaconValidators(
	...
	res, epochForCache, _, err = blockchain.getValidatorsFromCacheByEpoch(epoch, height, common.BeaconChainSyncID)
	if err != nil {
		Logger.log.Error(err)
	} else {
		return res, err
