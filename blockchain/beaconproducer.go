...
func (blockchain *BlockChain) NewBlockBeacon(curView *BeaconBestState, version int, proposer string, round int, startTime int64) (*BeaconBlock, error) {
	...
	newBeaconBeststate, err := beaconBestState.updateBeaconBestState(beaconBlock, blockchain, incognitokey.NewCommitteeChange())
	if err != nil {
		return nil, err
	}
	//============Build Header Hash=============
...
func (blockchain *BlockChain) GetShardState(
	...
	return shardStates, validStakeInstructions, validSwapInstructions, bridgeInstructions, acceptedRewardInstructions, validStopAutoStakeInstructions
}
