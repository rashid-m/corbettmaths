...
func (blockchain *BlockChain) NewBlockShard(curView *ShardBestState,
	...
	crossTransactions := blockchain.config.BlockGen.getCrossShardData(shardID, shardBestState, beaconProcessHeight)
...
//	   + Cross output coin
//	   + Cross Normal Token
func (blockGenerator *BlockGenerator) getCrossShardData(toShard byte, view *ShardBestState, currentBeaconHeight uint64) map[byte][]types.CrossTransaction {
	...
	for sid, v := range blockGenerator.syncker.GetCrossShardBlocksForShardProducer(view, nil) {
