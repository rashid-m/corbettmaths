		...
		for _, value := range keys {
			...
			if len(shardBlocks) >= len(block.Body.ShardState[shardID]) {
				...
				for index, shardBlock := range shardBlocks {
					...
					err := blockchain.config.ConsensusEngine.ValidateBlockCommitteSig(shardBlock.Hash(), currentCommittee, shardBlock.ValidationData, beaconBestState.ShardConsensusAlgorithm[shardID])
					if index == 0 && err != nil {
...
func (beaconBestState *BeaconBestState) Update(newBlock *BeaconBlock) error {
	...
	if newBlock.Header.Height == 1 {
		...
		beaconBestState.BeaconProposerIdx += newBlock.Header.Round
	}
