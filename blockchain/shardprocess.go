...
func (blockchain *BlockChain) InsertShardBlock(shardBlock *types.ShardBlock, shouldValidate bool) error {
	...
	if shouldValidate {
		...
		if err := blockchain.config.ConsensusEngine.ValidateBlockCommitteSig(shardBlock, curView.GetCommittee()); err != nil {
			Logger.log.Errorf("Validate block %v shard %v with committee %v return error %v", shardBlock.GetHeight(), shardBlock.GetShardID(), curView.GetCommittee(), err)
			return err
		}
	} else {
