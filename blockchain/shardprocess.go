...
func (oldBestState *ShardBestState) updateShardBestState(blockchain *BlockChain,
	...
	*ShardBestState, *committeestate.ShardCommitteeStateHash, *committeestate.CommitteeChange, error) {
	...
	for feature, _ := range config.Param().BlockTimeParam {
		Logger.log.Infof("[committee-state] feature: %s trigger feature: %+v", feature, shardBestState.TriggeredFeature[feature])
...
func (blockchain *BlockChain) processStoreAllShardStakerInfo(
	...
	for _, beaconBlock := range beaconBlocks {
		if (blockchain.IsFirstBeaconHeightInEpoch(beaconBlock.GetBeaconHeight())) && (beaconBlock.GetBeaconHeight() != 1) {
			...
			if err != nil {
				return fmt.Errorf("Beacon Consensus Root Hash of Height %+v not found ,error %+v", beaconBlock.GetHeight(), err)
			}
