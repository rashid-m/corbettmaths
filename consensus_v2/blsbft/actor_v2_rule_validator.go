...
func (c ConsensusValidatorLemma2) FilterValidProposeBlockInfo(bestViewProposeHash common.Hash, bestViewHeight uint64, finalViewHeight uint64, currentTimeSlot int64, proposeBlockInfos map[string]*ProposeBlockInfo) ([]*ProposeBlockInfo, []*ProposeBlockInfo, []string) {
	...
	for h, proposeBlockInfo := range proposeBlockInfos {
		...
		if c.chain.CalculateTimeSlot(proposeBlockInfo.block.GetBeaconHeight(), proposeBlockInfo.block.GetProposeTime()) != currentTimeSlot {
			continue
		}
		...
		if c.chain.CalculateTimeSlot(proposeBlockInfo.block.GetBeaconHeight(), proposeBlockInfo.block.GetProduceTime()) > currentTimeSlot {
			continue
		}
		...
		if proposeBlockInfo.block.GetFinalityHeight() != 0 {
			...
			if previousView.GetBlock().GetVersion() >= types.INSTANT_FINALITY_VERSION {
				previousProposeTimeSlot := c.chain.CalculateTimeSlot(previousView.GetBlock().GetBeaconHeight(), previousView.GetBlock().GetProposeTime())
				previousProduceTimeSlot := c.chain.CalculateTimeSlot(previousView.GetBlock().GetBeaconHeight(), previousView.GetBlock().GetProduceTime())
...
func (c ConsensusValidatorLemma2) ValidateConsensusRules(lastVotedBlock types.BlockInterface, isVoteNextHeight bool, proposeBlockInfo *ProposeBlockInfo) bool {
	...
	blockProduceTimeSlot := c.chain.CalculateTimeSlot(proposeBlockInfo.block.GetBeaconHeight(), proposeBlockInfo.block.GetProduceTime())
	lastBlockProduceTimeSlot := c.chain.CalculateTimeSlot(proposeBlockInfo.block.GetBeaconHeight(), lastVotedBlock.GetProduceTime())
	if blockProduceTimeSlot < lastBlockProduceTimeSlot {
		...
		c.chain.CalculateTimeSlot(proposeBlockInfo.block.GetBeaconHeight(), proposeBlockInfo.block.GetProposeTime()) > c.chain.CalculateTimeSlot(lastVotedBlock.GetBeaconHeight(), lastVotedBlock.GetProposeTime()) {
		c.logger.Infof("Block %+v is valid with rule 2, Block Propose Time %+v, < Last Block Propose Time %+v",
			proposeBlockInfo.block.FullHashString(),
			c.chain.CalculateTimeSlot(proposeBlockInfo.block.GetBeaconHeight(), proposeBlockInfo.block.GetProposeTime()),
			c.chain.CalculateTimeSlot(lastVotedBlock.GetBeaconHeight(), lastVotedBlock.GetProposeTime()))
...
func (c ConsensusValidatorLemma1) ValidateConsensusRules(lastVotedBlock types.BlockInterface, isVoteNextHeight bool, proposeBlockInfo *ProposeBlockInfo) bool {
	...
	blockProduceTimeSlot := c.chain.CalculateTimeSlot(proposeBlockInfo.block.GetBeaconHeight(), proposeBlockInfo.block.GetProduceTime())
	lastBlockProduceTimeSlot := c.chain.CalculateTimeSlot(lastVotedBlock.GetBeaconHeight(), lastVotedBlock.GetProduceTime())

	if blockProduceTimeSlot < c.chain.CalculateTimeSlot(lastVotedBlock.GetBeaconHeight(), lastVotedBlock.GetProduceTime()) {
		c.logger.Infof("Block %+v is valid with rule 1, Block Produce Time %+v, < Last Block Produce Time %+v",
			proposeBlockInfo.block.FullHashString(), blockProduceTimeSlot, lastBlockProduceTimeSlot)
		// blockProduceTimeSlot is smaller than voted block => vote for this block
		return true
	} else if blockProduceTimeSlot == c.chain.CalculateTimeSlot(lastVotedBlock.GetBeaconHeight(), lastVotedBlock.GetProduceTime()) &&
		c.chain.CalculateTimeSlot(proposeBlockInfo.block.GetBeaconHeight(), proposeBlockInfo.block.GetProposeTime()) > c.chain.CalculateTimeSlot(lastVotedBlock.GetBeaconHeight(), lastVotedBlock.GetProposeTime()) {
		c.logger.Infof("Block %+v is valid with rule 2, Block Propose Time %+v, < Last Block Propose Time %+v",
			proposeBlockInfo.block.FullHashString(),
			c.chain.CalculateTimeSlot(proposeBlockInfo.block.GetBeaconHeight(), proposeBlockInfo.block.GetProposeTime()),
