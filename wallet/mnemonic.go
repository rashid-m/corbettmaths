...
func (self *MnemonicGenerator) NewEntropy(bitSize int) ([]byte, error) {
...
func (self *MnemonicGenerator) EntropyFromMnemonic(mnemonic string) ([]byte, error) {
	mnemonicSlice, isValid := self.splitMnemonicWords(mnemonic)
	if !isValid {
	...
	entropy = self.padByteSlice(entropy, len(mnemonicSlice)/3*4)

	// generate the checksum once again, mask and ensure it equals the checksum we got from the mneomnic
	entropyChecksumBytes := self.computeChecksum(entropy)
...
func (self *MnemonicGenerator) NewMnemonic(entropy []byte) (string, error) {
	...
	entropy = self.addChecksum(entropy)
	...
	for i := sentenceLength - 1; i >= 0; i-- {
		...
		wordBytes := self.padByteSlice(word.Bytes(), 2)
...
func (self *MnemonicGenerator) MnemonicToByteArray(mnemonic string, raw ...bool) ([]byte, error) {
	var (
	...
	if !self.IsMnemonicValid(mnemonic) {
...
func (self *MnemonicGenerator) NewSeedWithErrorChecking(mnemonic string, password string) ([]byte, error) {
	_, err := self.MnemonicToByteArray(mnemonic)
	if err != nil {
		return nil, err
	}
	return self.NewSeed(mnemonic, password), nil
}

// NewSeed creates a hashed Seed output given a provided string and password.
// No checking is performed to validate that the string provided is a valid Mnemonic.
func (self *MnemonicGenerator) NewSeed(mnemonic string, password string) []byte {
...
func (self *MnemonicGenerator) IsMnemonicValid(mnemonic string) bool {
...
func (self *MnemonicGenerator) addChecksum(data []byte) []byte {
	// Get first byte of sha256
	hash := self.computeChecksum(data)
...
func (self *MnemonicGenerator) computeChecksum(data []byte) []byte {
	hasher := sha256.New()
	hasher.Write(data)
...
func (self *MnemonicGenerator) padByteSlice(slice []byte, length int) []byte {
...
func (self *MnemonicGenerator) compareByteSlices(a, b []byte) bool {
	if len(a) != len(b) {
		return false
	}
	for i := range a {
...
func (self *MnemonicGenerator) splitMnemonicWords(mnemonic string) ([]string, bool) {
	// Create a list of all the words in the Mnemonic sentence
	words := strings.Fields(mnemonic)
