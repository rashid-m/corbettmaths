...
func (mnemonicGen *MnemonicGenerator) EntropyFromMnemonic(mnemonic string) ([]byte, error) {
	...
	entropy = mnemonicGen.padByteSlice(entropy, len(mnemonicSlice)/3*4)

	// generate the checksum once again, mask and ensure it equals the checksum we got from the mneomnic
	entropyChecksumBytes := mnemonicGen.computeChecksum(entropy)
...
func (mnemonicGen *MnemonicGenerator) NewSeedWithErrorChecking(mnemonic string, password string) ([]byte, error) {
	_, err := mnemonicGen.MnemonicToByteArray(mnemonic)
	if err != nil {
		return nil, err
	}
	return mnemonicGen.NewSeed(mnemonic, password), nil
}
