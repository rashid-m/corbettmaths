...
//}
//
>>>>>>> b6347677c5eb36665f0ee0c6e68f06c757381859
...
func (wit *PKOneOfManyWitness) Prove( /*commitments [][]byte, indexIsZero int, commitmentValue []byte, index byte*/ ) (*PKOneOfManyProof, error) {
	...
	for j := 0; j <= n-1; j++ {
<<<<<<< HEAD
		xPoint := privacy.EllipticPoint{X:x}
		xPoint.ComputeYCoord()
		*x = *GenerateChallenge([]*privacy.EllipticPoint{&xPoint, cl[j], ca[j], cb[j], cd[j]})
=======
...
<<<<<<< HEAD
		xPoint := privacy.EllipticPoint{X:x}
		xPoint.ComputeYCoord()
		*x = *GenerateChallenge([]*privacy.EllipticPoint{&xPoint, pro.cl[j], pro.ca[j], pro.cb[j], pro.cd[j]})
=======
		*x = *GenerateChallengeFromByte([][]byte{x.Bytes(), cl[j].Compress(), ca[j].Compress(), cb[j].Compress(), cd[j].Compress()})
		// x.SetBytes(privacy.Elcm.GetHashOfValues([][]byte{x.Bytes(), cl[j], ca[j], cb[j], cd[j]}))
>>>>>>> b6347677c5eb36665f0ee0c6e68f06c757381859
...
func TestPKOneOfMany() bool {
	...
	witness.Set(commitments, randoms[indexIsZero], &indexIsZero, privacy.SND)
	//start := time.Now()
