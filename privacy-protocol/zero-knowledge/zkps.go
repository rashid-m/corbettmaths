...
func (wit *PaymentWitness) Build(hasPrivacy bool,
	...
	for i, inputCoin := range wit.inputCoins {
		...
		wit.ComInputValue[i] = privacy.PedCom.CommitAtIndex(big.NewInt(int64(inputCoin.CoinDetails.Value)), randInputValue[i], privacy.VALUE)
...
func (pro PaymentProof) Verify(hasPrivacy bool, pubKey privacy.PublicKey, db database.DatabaseInterface, shardID byte, tokenID *common.Hash) bool {
	// has no privacy
	if !hasPrivacy {
	...
	for i := 0; i < len(pro.ComInputOpeningsProof); i++ {
		...
		for j := 0; j < privacy.CMRingSize; j++ {
			commitmentBytes, err := db.GetCommitmentByIndex(tokenID, pro.OneOfManyProof[i].CommitmentIndexs[j], shardID)
			if err != nil {
