...
public:
    PRF_gadget(
    ...
    ) : gadget<FieldT>(pb, annotation_string), result(result) {
        ...
        block.reset(new block_variable<FieldT>(pb, {
            ...
            x,
            y
        }, annotation_string + std::string("PRF_block")));
        ...
        hasher.reset(new sha256_compression_function_gadget<FieldT>(
            ...
            block->bits,
            *result,
        annotation_string + std::string("PRF_hasher")));
    }
...
public:
    PRF_addr_a_pk_gadget(
        ...
        std::shared_ptr<digest_variable<FieldT>> result
    ) : PRF_gadget<FieldT>(pb, ZERO, 1, 1, 0, 0, a_sk, gen256zeroes(ZERO), result, "PRF_addr_a_pk_gadget") {}
};
...
public:
    PRF_nf_gadget(
        ...
        std::shared_ptr<digest_variable<FieldT>> result
    ) : PRF_gadget<FieldT>(pb, ZERO, 1, 1, 1, 0, a_sk, rho, result, "PRF_nf_gadget") {}
};
...
public:
    PRF_pk_gadget(
        ...
        bool nonce,
        std::shared_ptr<digest_variable<FieldT>> result
    ) : PRF_gadget<FieldT>(pb, ZERO, 0, nonce, 0, 0, a_sk, h_sig, result, "PRF_pk_gadget") {}
};
...
public:
    PRF_rho_gadget(
        ...
        bool nonce,
        std::shared_ptr<digest_variable<FieldT>> result
    ) : PRF_gadget<FieldT>(pb, ZERO, 0, nonce, 1, 0, phi, h_sig, result, "PRF_rho_gadget") {}
};
