...
func (wit *AggregatedRangeWitness) Prove() (*AggregatedRangeProof, error) {
	...
	var AggParam = newBulletproofParams(numValue)
...
func (proof *AggregatedRangeProof) Verify() bool {
	...
	//right1 := proof.cmsValue.ScalarMult(zSquare).Add(privacy.PedCom.G[privacy.VALUE].ScalarMult(deltaYZ)).Add(proof.T1.ScalarMult(x)).Add(proof.T2.ScalarMult(xSquare))
	//
