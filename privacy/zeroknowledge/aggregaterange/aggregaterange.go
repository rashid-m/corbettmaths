...
func (wit AggregatedRangeWitness) Prove() (*AggregatedRangeProof, error) {
	...
	// challenge y, z
	y := generateChallenge([][]byte{aggParam.cs.ToBytesS(), A.ToBytesS(), S.ToBytesS()})
	z := generateChallenge([][]byte{aggParam.cs.ToBytesS(), A.ToBytesS(), S.ToBytesS(), y.ToBytesS()})
	...
	x := generateChallenge([][]byte{aggParam.cs.ToBytesS(), proof.a.ToBytesS(), proof.s.ToBytesS(), proof.t1.ToBytesS(), proof.t2.ToBytesS()})
...
func (proof AggregatedRangeProof) Verify() (bool, error) {
	...
	y := generateChallenge([][]byte{aggParam.cs.ToBytesS(), proof.a.ToBytesS(), proof.s.ToBytesS()})
	z := generateChallenge([][]byte{aggParam.cs.ToBytesS(), proof.a.ToBytesS(), proof.s.ToBytesS(), y.ToBytesS()})
	zSquare := new(privacy.Scalar).Mul(z, z)
