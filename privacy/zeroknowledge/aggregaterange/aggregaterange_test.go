...
func TestInnerProduct(t *testing.T) {
	...
	for i := 0; i < n; i++ {
		a[i] = new(privacy.Scalar).SetUint64(10)
		b[i] = new(privacy.Scalar).SetUint64(20)
	}

	c, _ := innerProduct(a, b)
	assert.Equal(t, new(privacy.Scalar).SetUint64(400), c)
...
func TestPowerVector(t *testing.T) {
	twoVector := powerVector(new(privacy.Scalar).SetUint64(2), 5)
