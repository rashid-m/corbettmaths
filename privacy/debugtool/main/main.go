...
func GetBestBlock(tool *debugtool.DebugTool){
	fmt.Println("========== GET BEST BLOCKS INFO ==========")
	b, _ := tool.GetBestBlock()
	fmt.Println(string(b))
	fmt.Println("========== END GET BEST BLOCKS INFO ==========")
}
...
func main() {
	...
	for {
		...
		if args[0] == "test"{
			fmt.Println("Test createandsendtransaction")
			for i:=0;i<len(privateKeys);i+=1{
				shardID := GetShardIDFromPrivateKey(privateKeys[i])
				fmt.Printf("Send from %d (SHARD %d) to %d\n", i, int(shardID), (i+1)%len(privateKeys))
				if shardID == 0{
					tool = new(debugtool.DebugTool).InitLocal("9334")
				}else{
				...
				b, err := tool.CreateAndSendTransactionFromAToB(privateKeys[i], privateKeys[(i+1)%len(privateKeys)], "10000000")
				if err != nil{
					panic(err)
				}
				respond, err := debugtool.ParseResponse(b)
				if err != nil {
					panic(fmt.Sprintf("error at index %d: %v\n", i, err))
				}
				fmt.Printf("Send from %d to %d with error: %v\n", i, (i+1)%len(privateKeys), respond.Error)
			}
			fmt.Println("Finish test createandsendtransaction")
			tool = new(debugtool.DebugTool).InitLocal("9334")
		}

		if args[0] == "testpde"{
			fmt.Println("Test PDEContributePRV")
			for i:=0;i<len(privateKeys);i+=1{
				shardID := GetShardIDFromPrivateKey(privateKeys[i])
				fmt.Printf("PDEContributePRV %d (SHARD %d)\n", i, int(shardID))
				if shardID == 0{
					tool = new(debugtool.DebugTool).InitLocal("9334")
				}else{
					tool = new(debugtool.DebugTool).InitLocal("9338")
				...
				b, err := tool.PDEContributePRV(privateKeys[i], "10000000")
				if err != nil{
					panic(err)
				}
				respond, err := debugtool.ParseResponse(b)
				...
				fmt.Printf("PDEContributePRV %d (SHARD %d) with error: %v\n", i, int(shardID), respond.Error)
			}
			fmt.Println("Finish test PDEContributePRV")
			tool = new(debugtool.DebugTool).InitLocal("9334")
		}
