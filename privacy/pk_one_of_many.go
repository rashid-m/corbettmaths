...
func (pro *PKOneOfManyProtocol) Prove(commitments [][]byte, indexIsZero int,  commitmentValue []byte, rand []byte, index byte) (*PKOneOfManyProof, error) {
	...
	indexIsZeroBinary := ConvertIntToBinany(indexIsZero, n)
	...
	for k := 0; k < n; k++{
		...
		for i := 0; i < N; i++{
			...
			iBinary := ConvertIntToBinany(i, n)
	...
	zdInt := big.NewInt(0)
	zdInt.Exp(x, big.NewInt(int64(n)), Curve.Params().N)
	zdInt.Mul(zdInt, new(big.Int).SetBytes(rand))

	uxInt := big.NewInt(0)
	sumInt := big.NewInt(0)
	for k := 0; k < n; k++{
		uxInt.Exp(x, big.NewInt(int64(k)), Curve.Params().N)
		uxInt.Mul(uxInt, new(big.Int).SetBytes(u[k]))
		sumInt.Add(sumInt, uxInt)
		sumInt.Mod(sumInt, Curve.Params().N)
	}

	sumInt.Sub(Curve.Params().N, sumInt)

	zdInt.Add(zdInt, sumInt)
	zdInt.Mod(zdInt, Curve.Params().N)
...
func (pro *PKOneOfManyProtocol) Verify(commitments [][]byte, proof *PKOneOfManyProof, index byte, rand []byte) bool{
	...
	for i := 0; i < N; i++{
		...
		for j := 1; j <= n; j++{
			if iBinary[j-1] == 1{
				...
				fji.Mod(fji, Curve.Params().N)
			}

			exp.Mul(exp, fji)
			exp.Mod(exp, Curve.Params().N)
		}
	...
	for k := 0; k < n; k++{
		xk := big.NewInt(0)
		xk.Exp(x, big.NewInt(int64(k)), Curve.Params().N)

		xk.Sub(Curve.Params().N, xk)
	...
	rightValue3 := Pcm.CommitSpecValue(big.NewInt(0).Bytes(), proof.zd, index)
	rightPoint3, _ = DecompressCommitment(rightValue3)
...
func TestPKOneOfMany() {
	...
	res := pk.Verify(commitments, proof, SN_CM, randoms[indexIsZero])
	fmt.Println(res)
}
...
func GetCoefficient(iBinary []byte , k int, n int, a [][]byte, l []byte ) *big.Int{
	...
	for j:=1; j<=n; j++{
		...
		if iBinary[j-1] == 0 {
			fji = Poly{big.NewInt(0), big.NewInt(1)}.Sub(fj, Curve.Params().N)
		} else {
			fji = fj
		}
...
func TestGetCoefficient() {
	...
	lBinary := ConvertIntToBinany(l, n)
