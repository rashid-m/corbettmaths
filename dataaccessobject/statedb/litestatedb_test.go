...
func TestLiteStateDB(t *testing.T) {
	...
	txDB, err := NewLiteStateDB("./tmp/state", "txDB", common.Hash{}, db)
	if err != nil {
		t.Fatal(err)
	}
	...
	*/
	...
	restoreTxDB, err := NewLiteStateDB("./tmp/state", "txDB", aggHash5, db)
	if err != nil {
		t.Error(err)
	}
	...
	restoreTxDB, err = NewLiteStateDB("./tmp/state", "txDB", agghash5a, db)
	if err != nil {
		t.Fatal(err)
	}
	...
	restoreTxDB, err = NewLiteStateDB("./tmp/state", "txDB", newAgg, db)
	if err == nil {
		t.Fatal(err)
	}
	fmt.Println(err)
