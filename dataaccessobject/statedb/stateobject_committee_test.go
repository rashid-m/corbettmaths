...
func TestStateDB_GetMixCommitteePublicKey(t *testing.T) {
	...
	tempRootHash, tempM := storeCommitteeObjectOneShard(NextEpochShardCandidate, tempRootHash, CandidateChainID, from, to)
	for _, v := range tempM {
	...
	tempRootHash, tempM = storeCommitteeObjectOneShard(CurrentEpochShardCandidate, tempRootHash, CandidateChainID, from, to)
	for _, v := range tempM {
...
func TestStateDB_GetMixCommitteeState(t *testing.T) {
	...
	gotCurrentValidatorM, gotSubstituteValidatorM, gotNextEpochCandidateM, gotCurrentEpochCandidateM, _, _, _, _, _, _, _ := tempStateDB.getAllCommitteeState(ids)
	for _, id := range ids {
		temp, ok := gotCurrentValidatorM[id]
		if !ok {
			t.Fatalf("getAllCommitteeState want shard %+v", id)
