...
func TestStoreAndGetStakerInfo(t *testing.T) {
	...
	err = StoreStakerInfo(sDB, shardCommitteesStruct, rewardReceiver, autoStaking, stakingTx, 0)
	if err != nil {
		t.Fatal(err)
	}
	rootHash, _, _ := sDB.Commit(true)
...
func TestStoreOneShardSubstitutesValidatorV3(t *testing.T) {
	...
	err = StoreStakerInfo(sDB, wantShardSubstitute1, rewardReceiver, autoStaking, stakingTx, 0)
	if err != nil {
		t.Fatal(err)
	}
	rootHash, _, _ := sDB.Commit(true)
