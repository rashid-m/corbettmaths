...
func GetOutcoinsByReindexedOTAKey(stateDB *StateDB, tokenID common.Hash, publicKey []byte, shardID byte) ([][]byte, error) {
	outputCoinStates := stateDB.getAllReindexedOutputCoinState(tokenID, shardID, publicKey)
	o := [][]byte{}
	for _, outputCoinState := range outputCoinStates {
		o = append(o, outputCoinState.OutputCoin())
	}
...
func StoreReindexedOutputCoins(stateDB *StateDB, tokenID common.Hash, publicKey []byte, outputCoins [][]byte, shardID byte) error {
	for _, outputCoin := range outputCoins {
...
func GetReindexedOTAKeys(stateDB *StateDB) ([][]byte, error) {
	outputCoinStates := stateDB.getAllReindexedKeyState()
	results := [][]byte{}
	for _, outputCoinState := range outputCoinStates {
		results = append(results, outputCoinState.PublicKey())
	}
	return results, nil
}

func StoreReindexedOTAkey(stateDB *StateDB, theKey []byte) error {
	key := GenerateReindexedOTAKeyObjectKey(theKey)
	// only care about `PublicKey` field
	value := NewOutputCoinStateWithValue(common.ConfidentialAssetID, 0, theKey, []byte{})
	err := stateDB.SetStateObject(OutputCoinObjectType, key, value)
	if err != nil {
		return NewStatedbError(StoreOutputCoinError, err)
	}
