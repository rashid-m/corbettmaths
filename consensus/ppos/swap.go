			...
			case chainID := <-self.cSwapChain:
				{
					...
					producerPbk := common.EmptyString
					for _, committeeCandidatePbk := range committeeCandidateList {
						...
						producerPbk = committeeCandidatePbk
					}
					...
					if common.IndexOfStr(producerPbk, committee) >= 0 {
						continue
					}
					...
					reqSigMsg.(*wire.MessageSwapRequest).Candidate = producerPbk
				BeginSwap:
						...
						for {
							...
							case swapSig := <-self.cSwapSig:
								if common.IndexOfStr(swapSig.Validator, committee) >= 0 && swapSig.Validator == requesterPbk {
									...
									Logger.log.Info("SWAP validate signature ok from ", swapSig.Validator, producerPbk)
					...
					err := self.updateCommittee(producerPbk, chainID)
					if err != nil {
					...
					swapUpdMsg.(*wire.MessageSwapUpdate).Candidate = producerPbk
